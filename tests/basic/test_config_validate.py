...
def _clean_env():
    ...
    os.environ.update({k: v for k, v in original_env.items() if k in _HOME_ENV_VARS})
    try:
        yield
    finally:
        os.environ.clear()
...
@pytest.mark.parametrize(
    "mode", ["managed_identity", "certificate", "workload_identity"]
)
...
def test_describe_masks_entra_secrets():  # noqa
    ...
    with _clean_env():
        ...
        for key in ("azure_client_secret",):
            ...
            assert (
                "****" in described[key] or described[key] == "***"
